...
public class StudentUpdateFrm extends JFrame {
	...
	public void loadData() {
		...
		Student bean = studentService.load(pk.longValue());
		if (bean == null) {
		...
		if (bean != null) {
			...
			cboRoomId.getModel().setSelectedItem(new IdText(bean.getRoomId(), ""));
			this.repaint();
		}
	}

	ClassroomService roomService = new ClassroomServiceImpl();
	...
	private void bindComBoxData() {
		...
		java.util.List<Classroom> listB = roomService.list();
		for (Classroom item : listB) {
			idTextB.add(new IdText(item.getId(), item.getRoomName()));
		}
		IdTextModel modelB = new IdTextModel(idTextB);
		cboRoomId.setModel(modelB);
		cboRoomId.setSelectedIndex(0);
	}
	...
	private void customInitUI() {
		...
		lblRoomId.setText("班级ID：");
		lblRoomId.setBounds(75, 220, 80, 30);
		container.add(lblRoomId);

		cboRoomId = new JComboBox<IdText>();
		cboRoomId.setBounds(160, 220, 120, 30);
		container.add(cboRoomId);

		btnSubmit = new JButton();
		btnSubmit.setText("提交");
		btnSubmit.setBounds(80, 265, 60, 30);
		container.add(btnSubmit);

		btnReset = new JButton();
		btnReset.setText("重置");
		btnReset.setBounds(160, 265, 60, 30);
...
//		lblMsg.setText("提示信息");
	...
	private void btnSubmit_click(ActionEvent e) {
		...
		IdText itemRoomId = (IdText) cboRoomId.getSelectedItem();
		Long roomId = itemRoomId.getId();
